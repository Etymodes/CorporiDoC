...
真实 MediaPipe 后端现可从“姿态”标签页运行，生成版本化 33 点 CSV，并保留未检出帧、逐点
visibility/presence、归一化/像素坐标和米制世界坐标。任务完成只表示视频处理完成，不表示患者
姿态正常或形成临床结论。

MediaPipe 任务还可勾选生成骨架叠加 MP4。可见度不足的点不会强行连线，未检出帧会明确标红；
画面固定显示模型短哈希和 `NOT CLINICALLY VALIDATED`，输出文件再次登记 SHA-256。

M5 第一小 Demo 已加入 Video-EEG 时间对齐核心：使用至少两个同步锚点拟合
`t_eeg = slope * t_video + offset`，支持双向换算并报告 RMS/最大残差。它不读取患者 EEG、
不自动判定同步质量；后续将持久化锚点并用 `ffprobe` 读取视频真实 PTS。

后端路线不再局限于 DLC：MediaPipe/MMPose 用于通用人体粗标，OpenCV 光流与 TAPIR/TAPNext
用于短视频半自动点传播，DLC/Lightning Pose/MMPose 用于 TiantanDoC 专病训练，可靠轨迹建立后
再评估 MotionBERT、PoseC3D/ST-GCN 等运动模式模型。各后端共享版本化关键点与时间基协议。
